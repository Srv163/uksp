...
const rows = data.projects.map(p=>`<article class="project" aria-labelledby="project-${esc(p.id)}">
  ...
  <details class="project-details"><summary><span>Даты и суммы сделки</span><span class="project-duration">${days(p)} ${dayWord(days(p))} владения</span><span class="project-expand" aria-hidden="true">+</span></summary><div class="project-detail-grid"><div><span>Вход</span><time datetime="${esc(p.purchaseDate)}">${date(p.purchaseDate)}</time><b>${number(p.purchasePrice)} ₽</b></div><div><span>Продажа</span><time datetime="${esc(p.saleDate)}">${date(p.saleDate)}</time><b>${number(p.salePrice)} ₽ <small>без НДС</small></b></div><p>Срок владения — от даты полной оплаты до даты продажи. Суммы в заголовке округлены до сотых миллиона.</p></div></details>
</article>`).join('\n');
